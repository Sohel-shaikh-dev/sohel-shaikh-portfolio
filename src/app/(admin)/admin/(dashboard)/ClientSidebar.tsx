...
import { Menu, X, LayoutDashboard, FolderKanban, FileText, Award, Briefcase, MessageSquare, LogOut, Settings, Link as LinkIcon } from 'lucide-react'
...
export function ClientSidebar({ signOutAction }: { signOutAction: () => void }) {
  ...
  const links = [
    ...
    { name: 'Footer Social Links', href: '/admin/social-links', icon: <LinkIcon size={18} /> },
    { name: 'Dev Social Links', href: '/admin/developer-social-links', icon: <LinkIcon size={18} /> },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <motion.aside 
        className={`fixed lg:sticky top-0 left-0 h-screen w-64 bg-card border-r border-white/5 p-6 flex flex-col z-40 overflow-y-auto transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
      >
