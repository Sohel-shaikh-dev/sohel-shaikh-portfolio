...
export async function updateSiteSettings(formData: any) {
  ...
  // We no longer validate social link URLs here as they are managed in the Social Links module
  ...
  // Track old CV path for cleanup AFTER successful DB update
  const oldCvPath = existing?.cv_pdf_path;
  const newCvPath = formData.cv_pdf_path;
  // Delete old CV if a new one was uploaded or if it was cleared
  const shouldDeleteOldCv = oldCvPath && oldCvPath !== newCvPath;
  ...
  if (result.error) {
    console.error('[updateSiteSettings] DB Update Failed:', result.error.message);
    return { success: false, error: result.error.message }
  }

  // Transaction Safe: Only delete from storage if DB update succeeded
  if (shouldDeleteOldCv) {
    const { error: removeError } = await supabase.storage.from('portfolio-media').remove([oldCvPath])
    if (removeError) {
      console.error(`[updateSiteSettings] Failed to delete old CV from storage: ${oldCvPath}`, removeError);
    } else {
      console.log(`[updateSiteSettings] Successfully deleted old CV from storage: ${oldCvPath}`);
    }
  }
