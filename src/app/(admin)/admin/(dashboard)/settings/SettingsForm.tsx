...
export function SettingsForm({ initialData }: { initialData: any }) {
  ...
  const [formData, setFormData] = useState({
    stats_projects_done: data.stats_projects_done || '80+',
    stats_happy_clients: data.stats_happy_clients || '50+',
    stats_success_rate: data.stats_success_rate || '99%',
      ...
      >
        ...
        <div>
          ...
          {formData.cv_pdf_path ? (
            <div className="w-full rounded-2xl px-5 py-4 bg-background border border-white/5 neumorphic-inner flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <span className="text-sm text-gray-300 truncate w-full sm:w-auto">Uploaded: {formData.cv_pdf_path}</span>
             ...
             <FileUpload 
              ...
              fileType="document"
              maxSizeMB={10}
              onUploadComplete={(path) => setFormData({...formData, cv_pdf_path: path})} 
            />
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="text-[12px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-2 block ml-2">Email</label>
            <ParticleWrapper>
              <input 
                type="email"
