        ...
        <AnimatedCard key={msg.id} index={index} className="p-6 flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0">
            <div className="flex items-start gap-4 w-full sm:w-auto overflow-hidden">
              <div className={`p-3 rounded-xl border border-white/5 flex-shrink-0 ${msg.status === 'UNREAD' ? 'bg-primary/20 text-primary neumorphic-inner' : 'bg-background text-gray-400 neumorphic'}`}>
                {msg.status === 'UNREAD' ? <Mail size={20} /> : <MailOpen size={20} />}
              </div>
              <div className="min-w-0 flex-1">
                <h4 className="font-bold text-lg mb-1 flex flex-wrap items-center gap-2">
                  <span className="truncate">{msg.sender_name}</span> 
                  <span className="text-sm font-normal text-gray-400 truncate">{msg.sender_email}</span>
                </h4>
                <p className="text-xs text-gray-400 font-bold uppercase tracking-widest truncate">{msg.subject}</p>
              </div>
            </div>
            <div className="text-xs text-gray-500 font-bold self-start sm:self-auto flex-shrink-0">
