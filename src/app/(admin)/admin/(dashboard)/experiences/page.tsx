        ...
        <div>
          ...
          <div className="flex flex-col gap-4">
            {experiences?.map((exp, index) => (
              <AnimatedCard key={exp.id} index={index} className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 group">
                <div className="w-full sm:w-auto overflow-hidden">
                  <div className="font-bold text-xl mb-1 group-hover:text-primary transition-colors truncate">{exp.job_title}</div>
                  <div className="text-xs text-gray-400 uppercase tracking-widest font-bold truncate">{exp.company_name}</div>
                </div>
                <div className="flex flex-wrap sm:flex-nowrap items-center gap-4 sm:ml-4 w-full sm:w-auto">
                  <div className="text-xs font-bold px-4 py-2 rounded-full bg-background neumorphic-inner text-gray-300 whitespace-nowrap">
                    {exp.start_date 
                      ? (exp.is_current ? `${exp.start_date} - Present` : `${exp.start_date} - ${exp.end_date || 'Present'}`)
                      : exp.date_range}
                  </div>
